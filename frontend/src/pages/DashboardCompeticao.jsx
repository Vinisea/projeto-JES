...
export default function DashboardCompeticao() {
  ...
  return (
    <main className="page-shell competition-page">
      <section className="competition-header"><div><span className="eyebrow">VISÃO GERAL</span><h1>Dashboard da competição</h1><p>Acompanhe o andamento dos Jogos Internos JES 2026.</p></div><Link className="secondary-button" to="/admin/partidas">Gerenciar partidas</Link></section>
      ...
      <section className="competition-columns">
        <article className="competition-panel"><div className="competition-panel-heading"><div><span className="eyebrow">AGENDA</span><h2>Próximas partidas</h2></div><Link to="/admin/partidas">Ver todas</Link></div>{proximas.length ? proximas.map((partida) => <div className="competition-match" key={partida.id}><div className="competition-match-time"><strong>{partida.horario}</strong><small>{partida.local}</small></div><div><span>{partida.modalidade} · {partida.grupo}</span><strong>{partida.equipeA} <em>×</em> {partida.equipeB}</strong></div></div>) : <div className="empty-state">Nenhuma próxima partida.</div>}</article>
